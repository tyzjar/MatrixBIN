...
char *key;
  if( (key=regname(reg))==NULL ){
	if( verb ) fprintf(stderr,"Error: register %x not found\n",reg);
	return -1;
  }
  jresp=cJSON_CreateObject();
  sprintf(val,"%d",v);
  if( !cJSON_AddStringToObject(jresp,key,val) ){
	if( verb ) fprintf(stderr,"Error: cJSON_AddStringToObject(response)\n");
	cJSON_Delete(jresp);
	return -2;
  }
...
// car
	...
	strcpy(car.id,jwgt->valuestring);
// seal
  }
*/
/// not used yet
  ...
  wgt.d[1]=0; // stability
///
  wgt.wgt.w[0]=atoi(jwgt->valuestring); // brutto
...
char clientid[]="modbus";
  ...
  if( mosq==NULL ){
//#ifdef VERBOSE
	if( verb ) fprintf(stderr, "Error: Out of memory.\n");
//#endif
	return NULL;
  }
...
//#ifdef VERBOSE
	if( verb ) fprintf(stderr,"Error: %s\n",mosquitto_strerror(rc));
//#endif
	return NULL;
  }
