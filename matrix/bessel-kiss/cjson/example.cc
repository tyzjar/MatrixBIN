...
// В функции обработки соединения создаем json конфигурации (согласно конфигурационному файлу) и пубикуем ее в "calibration/configuration"
...
int createConfiguration( char** szConfiguration)
{
	...
	for( int i=0; i<4; ++i) {
		...
		char buf[50];
		snprintf( buf, 50, "%d", devices[i].id);
